...
const uint CAPTURE_LED_BASE = 24;
const uint CAPTURE_LED_COUNT = 8;
...
void logic_analyser_arm(PIO pio, uint sm, uint dma_chan, uint32_t *capture_buf, size_t capture_size_words,
    ...
    if (trigger_pin >= 0) {
        // Wait for trigger condition on trigger pin before starting capture    
        pio_sm_exec(pio, sm, pio_encode_wait_gpio(trigger_level, trigger_pin));
    }
    else {
        // No trigger, just start immediately
    }
...
void led_output_init(PIO pio, uint sm, float div) {
    // A simple program to output the state of the least significant bit of the
    // OSR to a pin. We will use this to show when the logic analyser is
    // triggered and running.
    uint16_t output_prog_instr = pio_encode_out(pio_pins, 8);
    struct pio_program output_prog = {
            .instructions = &output_prog_instr,
            .length = 1,
            .origin = -1
    };
    uint offset = pio_add_program(pio, &output_prog);

    pio_sm_config c = pio_get_default_sm_config();
    sm_config_set_out_pins(&c, CAPTURE_LED_BASE, CAPTURE_LED_COUNT);

        // Set the pins' GPIO function (connect PIO to the pad)
    for (int i = CAPTURE_LED_BASE; i < CAPTURE_LED_BASE+CAPTURE_LED_COUNT; i++) {
        pio_gpio_init(pio, i);
    }
    // Set pins as GPIO outputs ; This is the hardware direction, not the PIO pin control
    pio_sm_set_consecutive_pindirs(pio, sm, CAPTURE_LED_BASE, CAPTURE_LED_COUNT, true);

    sm_config_set_wrap(&c, offset, offset);
    sm_config_set_clkdiv(&c, div);
    sm_config_set_fifo_join(&c, PIO_FIFO_JOIN_TX);
    sm_config_set_out_shift(&c, true, true, CAPTURE_LED_COUNT);
    pio_sm_init(pio, sm, offset, &c);

    pio_sm_set_enabled(pio, sm, true);
}

void led_output_set(PIO pio, uint sm, bool on) {
    pio_sm_put(pio, sm, on ? 0xff : 0u);
}

int main() {
    stdio_init_all();
    sleep_ms(1000); // use minicom to view
    ...
    // set for 4MHz means 31.25f
    logic_analyser_init(pio0, sm0, CAPTURE_PIN_BASE, CAPTURE_PIN_COUNT, 1.f); 

    led_output_init(pio1, sm1, 1.f);
    ...
    if (true)
    {
        ...
        // logic_analyser_arm(pio, sm, dma_chan, capture_buf, buf_size_words, -1, true);
        for (int i = 2; i < CAPTURE_PIN_COUNT; ++i)
        {
            pio_gpio_init(pio0, CAPTURE_PIN_BASE + i);
            gpio_pull_up(CAPTURE_PIN_BASE + i);
        }
        pio_sm_set_consecutive_pindirs(pio0, sm0, CAPTURE_PIN_BASE+2, CAPTURE_PIN_COUNT-2, false);

        led_output_set(pio1, sm1, !led_off);
...
//    else
    ...
    start = time_us_64();
           logic_analyser_arm(pio0, sm0, dma_chan, capture_buf, buf_size_words, CAPTURE_PIN_BASE, true);
        // PWM example: -----------------------------------------------------------
    ...
    printf("Transfer complete in %llu us (%0.5g MHz) (%0.5g million words/s)\n", elapsed, (float)1.0f/elapsed, (float)buf_size_words*1.0f/elapsed);
    // printf("Throughput: %.0f bytes/us (TX + RX combined)\n", 
    //        (64.0f * 2) / elapsed);
